...
class TCAV(object):
    ...
    @staticmethod
    def get_directional_dir(mymodel, target_class, concept, cav, class_acts, examples):
    ...
    def run(
        self, num_workers=10, run_parallel=False, overwrite=False, return_proto=False
    ):
        ...
        if run_parallel:
            ...
            for i, param in enumerate(self.params):
                tf.compat.v1.logging.info(
                    "Running param %s of %s" % (i, len(self.params))
                )
                # tf.compat.v1.logging.info("Hello: We are at %s" % (vars(param))) #print parameters
                results.append(
